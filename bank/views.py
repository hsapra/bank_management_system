...
def admin_home(request):
	cache.clear()
	if 'perID' in request.session and request.session['perID'] is not None and request.session['isAdmin']:
		return render(request, 'bank/admin_home.html', {'perID': request.session['perID'], 'role': 'Admin'})
	else:
		messages.error(request, 'Access Denied. Please Log In with the correct credentials')
		return redirect('/bank/')
...
def manager_home(request):
	cache.clear()
	if 'perID' in request.session and request.session['perID'] is not None and request.session['isManager']:
		return render(request, 'bank/manager_home.html', {'perID': request.session['perID'], 'role': 'Manager'})
	else:
		messages.error(request, 'Access Denied. Please Log In with the correct credentials')
		return redirect('/bank/')
...
def customer_home(request):
	cache.clear()
	if 'perID' in request.session and request.session['perID'] is not None and request.session['isCustomer']:
		return render(request, 'bank/customer_home.html', {'perID': request.session['perID'], 'role': 'Customer'})
	else:
		messages.error(request, 'Access Denied. Please Log In with the correct credentials')
		return redirect('/bank/')
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			if len(Employee.objects.filter(perid = form.cleaned_data['perID'])) == 0 and len(SystemAdmin.objects.filter(perid = form.cleaned_data['perID'])) == 0:
				...
				messages.error(request, "Cannot start customer role for existing employee/admin")
	else:
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			if len(SystemAdmin.objects.filter(perid = form.cleaned_data['perID'])) == 0 and len(Customer.objects.filter(perid = form.cleaned_data['perID'])) == 0:
				...
				messages.error(request, "Cannot start customer role for existing customer/admin")
	...
	if request.method == 'POST':
		...
		if 'employee' in request.POST and form1.is_valid():
			if len(Employee.objects.filter(perid = form1.cleaned_data['perID'])) >= 1:
				employee_bank_ids = Workfor.objects.filter(perid = form1.cleaned_data['perID']).values_list('bankid', flat=True)
				can_happen = True
				for bank_id in employee_bank_ids:
					if len(Workfor.objects.filter(bankid = bank_id)) == 1:
						can_happen = False
				if can_happen:
					if len(Bank.objects.filter(manager_id = form1.cleaned_data['perID'])) == 0:
		...
		if 'customer' in request.POST and form2.is_valid():
			customer_accessed_accounts = Access.objects.filter(perid = form2.cleaned_data['perID']).values_list('bankid', 'accountid')
			can_happen = True
			...
			if can_happen:
				...
				messages.error(request, "Customer is connected to an account as a sole owner")
...
def add_account_access(request):
	if 'perID' in request.session and request.session["perID"] is not None and request.session["isAdmin"]:
			...
			if 'add' in request.POST and form1.is_valid():
				...
				if not bankAcct_created:
					...
					if accountType == 'Checking':
						...
						InterestBearing.objects.filter(bankid = bankAcct, accountid = bankAcct.accountid,).update(interest_rate = form1.cleaned_data['interestRate'], dtdeposit = now)
						savings, _ = Savings.objects.get_or_create(
							bankid = interest,
							accountid = interest.accountid
						)
						Savings.objects.filter(bankid = interest,accountid = interest.accountid).update(minbalance = form1.cleaned_data['minbalance'])
					else:
						interest, _ = InterestBearing.objects.get_or_create(
							bankid = bankAcct,
							accountid = bankAcct.accountid,
						...
						Market.objects.filter(bankid = interest,accountid = interest.accountid).update(maxwithdrawals = form1.cleaned_data['maxwithdrawals'])
				...
				if len(Access.objects.filter(bankid=bankid, accountid=accountid, perid=sharerid)) > 0:
					messages.error(request, "Access already exists")
				else:
					if len(BankAccount.objects.filter(bankid=bankid, accountid=accountid)) == 0:
						messages.error(request, "Access does not exists")
					else:
						bankAcct = BankAccount.objects.get(bankid=bankid, accountid=accountid)
		...
		return render(request, "bank/add_account_access_admin.html", {"form1": form1, "form2": form2})
	elif 'perID' in request.session and request.session["perID"] is not None and request.session["isCustomer"]:
		if request.method == "POST":
			# create a form instance and populate it with data from the request:
			...
			if form.is_valid():
				...
				if len(Access.objects.filter(bankid=bankid, accountid=accountid, perid=request.session['perID'])) == 0:
					messages.error(request, "Cannot modify an account you don't have access to!")

				elif len(Access.objects.filter(bankid=bankid, accountid=accountid, perid=sharerid)) > 0:
					messages.error(request, "Access already exists")
				else:
					if len(BankAccount.objects.filter(bankid=bankid, accountid=accountid)) == 0:
						messages.error(request, "Access does not exists")
...
def remove_account_access(request):
	if 'perID' in request.session and request.session["perID"] is not None and (request.session["isAdmin"] or request.session["isCustomer"]):
		if request.method == "POST":
			# create a form instance and populate it with data from the request:
		...
		if form.is_valid():
			...
			# insert into interest_bearing_fees values (ip_bankID, ip_accountID, ip_fee_type);
				new_interest_fee , created = InterestBearingFees.objects.get_or_create(
					bankid = InterestBearing.objects.get(bankid = formBankID, accountid = formAccountID),
					accountid = form.cleaned_data['accountID'],
		...
		if request.method == 'POST':
			...
			if form.is_valid():
				...
				if len(Access.objects.filter(bankid = bankID, accountid = accountID, perid = perID)) == 0:
					...
					protectionBank = Checking.objects.get(bankid = bankID, accountid = accountID).protectionbank
					protectionAccount = Checking.objects.get(bankid = bankID, accountid = accountID).protectionaccount
					if len(BankAccount.objects.filter(bankid = protectionBank, accountid = protectionAccount) == 0):
						overdraft_balance = 0
					else:
						overdraft_balance = BankAccount.objects.get(bankid = protectionBank, accountid = protectionAccount).balance or 0
					if amount > (balance + overdraft_balance):
						...
						Access.objects.filter(bankid = protectionBank, accountid = protectionAccount).update(dtaction = current_date_time)
						Checking.objects.filter(bankid = bankID, accountid = accountID).update(dtoverdraft = current_date_time)
						Checking.objects.filter(bankid = bankID, accountid = accountID).update(amount = amount - balance)
				...
				if (
					...
					if is_checking(
						...
						if form.cleaned_data["transferAmount"] > (
							...
							Checking.objects.filter(
								bankid=bankID, accountid=accountID
							).update(amount=(cleaned_data["transferAmount"] - balance))
							messages.success(
...
def pay_employees(request):
	if 'perID' in request.session and request.session["perID"] is not None and (request.session["isAdmin"] or request.session["isManager"]):
		if request.method == "POST":
			...
			for employee, _ in employees:
				...
				if len(Workfor.objects.filter(perid=employee)) == 0:
					...
					employee_salary_divison[employee] = math.floor((emp.salary or 0)/len(Workfor.objects.filter(perid=employee)))
			
			for employee in employee_salary_divison.keys():
				salary = employee_salary_divison[employee]
				for bankid in Access.objects.filter(perid = employee).values_list('bankid', flat=True):
